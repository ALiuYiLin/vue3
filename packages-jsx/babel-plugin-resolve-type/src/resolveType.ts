...
import {
  type ImportBinding,
  type ScriptCompileContextLite as ScriptCompileContext,
  type SFCScriptCompileOptionsLite as SFCScriptCompileOptions,
  createCache,
...
// This file is vendored from @vue/compiler-sfc's `script/resolveType.ts` so
// that the standalone @vue/babel-plugin-resolve-type has no compiler
// dependency. It runs only inside a Node-based Babel plugin, so the browser /
// test build branches below are pinned to fixed values.
const __CJS__ = true
...
function resolveMappedType(
  ...
  if (node.nameType) {
    ...
    scope = createChildScope(scope)
    Object.assign(scope.types, { ...typeParameters, [name]: node.constraint })
...
function resolveIndexType(
  ...
  for (const key of keys) {
    ...
    const targetType =
      prop && 'typeAnnotation' in prop
        ? prop.typeAnnotation?.typeAnnotation
        : undefined
    if (targetType) {
      ;(targetType as TSType & MaybeWithScope)._ownerScope = prop._ownerScope
...
function qualifiedNameToPath(node: TSEntityName): string[] {
...
function parseFile(
  ...
  if (ext === '.vue') {
    // `.vue` sources are not supported by the standalone
    // @vue/babel-plugin-resolve-type — parsing them requires the SFC parser
    // from @vue/compiler-sfc, which this fork no longer ships. Imported types
    // are expected to live in regular `.ts` / `.d.ts` files.
    return []
  }
  return []
}
...
function moduleDeclToScope(
  ...
  // Babel 8 always represents the body as a TSModuleBlock; nested namespace
  // declarations appear as statements inside it.
...
function recordType(
  ...
  switch (node.type) {
    ...
    case 'TSModuleDeclaration': {
      ...
      const id = overwriteId || getId(node.id as Identifier | StringLiteral)
      ...
      if (existing) {
        ...
        if (node.type === 'TSInterfaceDeclaration') {
          ;(existing as TSInterfaceDeclaration).body.body.push(
            ...node.body.body,
          )
        } else if (node.type === 'TSEnumDeclaration') {
          ;(existing as TSEnumDeclaration).body.members.push(
            ...node.body.members,
          )
        } else {
          const decl = node as unknown as TSModuleDeclaration
          ;(existing as unknown as TSModuleDeclaration).body.body.push(
            ...decl.body.body,
          )
        }
      } else {
...
function mergeNamespaces(to: TSModuleDeclaration, from: TSModuleDeclaration) {
  // Babel 8 always represents namespace bodies as a TSModuleBlock, so the
  // old Babel-7 declaration/block merging branches are no longer needed.
  ...
  try {
    switch (node.type) {
      ...
      case 'TSMappedType': {
        ...
        const { typeAnnotation, constraint, key } = node
        if (
          typeAnnotation &&
...
function resolveExtractPropTypes(
  ...
  for (const key in props) {
    ...
    res.props[key] = reverseInferType(
      raw.key,
      (raw as TSPropertySignature).typeAnnotation!.typeAnnotation,
      scope,
    )
  }
  return res
