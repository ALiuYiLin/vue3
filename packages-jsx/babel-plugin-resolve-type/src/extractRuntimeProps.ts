...
import { BindingTypes, isFunctionType, unwrapTSNode } from './types.ts'

// Vendored from @vue/compiler-sfc's `script/defineProps.ts` — only the
// runtime extraction parts (the `defineProps` macro processing itself is not
// needed outside the SFC compiler).
...
export function extractRuntimeProps(
  ...
  for (const prop of props) {
    ...
    // register bindings (only available in an SFC compile context)
    const bindingMetadata = (
      ctx as { bindingMetadata?: Record<string, string> }
    ).bindingMetadata
    if (bindingMetadata && !(prop.key in bindingMetadata)) {
      bindingMetadata[prop.key] = BindingTypes.PROPS
    }
  }
